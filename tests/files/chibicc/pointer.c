...
int main() {
  ...
  ASSERT(5, ({ int x=3; int y=5; *(&x-1); }));  // Clang: y is before x
  ASSERT(3, ({ int x=3; int y=5; *(&y+1); }));  // Clang: x is after y
  ASSERT(5, ({ int x=3; int y=5; *(&x-1); }));  // Clang: y is before x
  ASSERT(5, ({ int x=3; int *y=&x; *y=5; x; }));
  ASSERT(7, ({ int x=3; int y=5; *(&x-1)=7; y; }));  // Clang: y is before x
  ASSERT(7, ({ int x=3; int y=5; *(&y+1)=7; x; }));  // Clang: x is after y
  ASSERT(5, ({ int x=3; (&x+2)-&x+3; }));  // Pointer arithmetic (layout-independent)
  ASSERT(8, ({ int x, y; x=3; y=5; x+y; }));
  // ASSERT(8, ({ int x=3, y=5; x+y; }));  // TODO: comma-separated declarations with initializers not yet supported

  ASSERT(3, ({ int x[2]; int *y=&x; *y=3; *x; }));

  // TODO: Multiple statement expressions with arrays cause segfaults
